...
func CheckRateLimit(ctx context.Context, redisClient *redis.Redis, retryAfter int, email, codeType string) error {
...
func CheckRegisterLogic(ctx context.Context, usersModel model.UsersModel, mqClient svc.KqPusherClient, remindRegisteredType string, email string) (shouldContinue bool, err error) {
...
// 返回 shouldContinue: 是否继续发送验证码
func CheckResetPasswordLogic(ctx context.Context, usersModel model.UsersModel, email string) (shouldContinue bool, err error) {
	_, err = usersModel.FindOneByEmail(ctx, email)
	if err == sqlx.ErrNotFound {
		// 邮箱不存在，返回错误，不发送验证码
...
func GenerateAndSaveVerifyCode(ctx context.Context, redisClient *redis.Redis, expireIn int, email, codeType string) string {
...
func SendVerifyCodeToMQ(ctx context.Context, mqClient svc.KqPusherClient, email, code, codeType string) error {
...
func CleanupVerifyCodeAll(ctx context.Context, redisClient *redis.Redis, email, codeType string) {
...
func CheckBusinessLogic(ctx context.Context, usersModel model.UsersModel, mqClient svc.KqPusherClient, cfg config.VerifyCodeConfig, email, codeType string) (shouldContinue bool, err error) {
	switch codeType {
